...
@endphp
<div class="project-item animate-fade-in" data-category="{{ $category }}">
    <button type="button" class="project-card group block w-full overflow-hidden rounded-2xl border border-border bg-card/40 text-left transition-all duration-300 hover:-translate-y-1 hover:border-primary/40 hover:shadow-xl hover:shadow-primary/10"
        data-project='{!! json_encode($projectData) !!}'>
        ...
        <div class="p-6">
            <span class="inline-flex items-center gap-1 rounded-full bg-primary/15 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-primary">{{ $category }}</span>
            <h3 class="mt-3 font-display text-lg font-bold tracking-tight">{{ $title }}</h3>
