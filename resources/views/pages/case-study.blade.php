...
@section('content')
    ...
    <section class="px-6 pb-12">
        <div class="mx-auto max-w-5xl">
            @php
                $metrics = $study['metrics'] ?? [['value' => ($study['metric'] ?? '—'), 'label' => ($study['metric_label'] ?? '')]];
            @endphp
    ...
    <section class="px-6 pb-12">
        <div class="mx-auto max-w-5xl">
            ...
            <div class="mt-4 leading-relaxed text-muted-foreground">
                @if(isset($study['results']) && is_array($study['results']))
    ...
    <section class="px-6 pb-20">
        <div class="mx-auto flex max-w-5xl flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
            <a href="{{ route('preview.show', $slug) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 rounded-full border border-primary/40 px-6 py-3 text-xs font-semibold uppercase tracking-widest text-primary transition-all duration-300 hover:bg-primary/10">
                View Live Demo
