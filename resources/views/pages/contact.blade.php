...
<section style="background: linear-gradient(135deg, #f8f6f3 0%, #f0ede8 100%); padding: 4rem 0 2rem;">
    <div class="container">
        <div class="row g-4">
            <div class="col-lg-5">
                <p class="fw-semibold mb-2" style="color: var(--md-sys-color-primary); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.08em;">{{ config('contact_page.eyebrow') }}</p>
                <h1 class="display-6 fw-bold mb-3" style="font-family: 'Outfit', sans-serif; color: var(--md-sys-color-on-surface); font-size: 2.25rem;">{{ config('contact_page.headline') }}</h1>
                    ...
                    <div class="card-body p-4">
                        ...
                        <ul class="list-unstyled mb-0" style="font-size: 0.9375rem; color: var(--md-sys-color-on-surface-variant);">
                            <li class="d-flex align-items-center gap-3 mb-3">
                                ...
                                <a href="mailto:{{ config('contact.email') }}" style="color: var(--md-sys-color-on-surface-variant); text-decoration: none;">{{ config('contact.email') }}</a>
                            </li>
                            <li class="d-flex align-items-center gap-3 mb-3">
                                <div class="rounded-circle d-flex align-items-center justify-content-center" style="width: 40px; height: 40px; background: var(--md-sys-color-primary-container); color: var(--md-sys-color-on-primary-container);">
                                ...
                                <a href="tel:{{ config('contact.phone') }}" style="color: var(--md-sys-color-on-surface-variant); text-decoration: none;">{{ config('contact.phone_display') }}</a>
                            </li>
...
document.getElementById('contactForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        responseDiv.style.display = 'block';
        responseDiv.className = 'alert alert-danger rounded-3';
        responseDiv.textContent = 'Network error. Please email us directly at ' + '{{ config('contact.email') }}';
    } finally {
        btn.disabled = false;
